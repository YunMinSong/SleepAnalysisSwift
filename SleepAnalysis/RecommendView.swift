            ...
            ZStack {
                ...
                VStack(alignment: .leading) {
                    ...
                    ZStack {
                        ...
                        Path { path in
                            path.move(to: CGPoint(x: 180, y: 92.5))
                            path.addArc(center: .init(x: 180, y: 92.5), radius: 75, startAngle: Angle(degrees: timeToAngle(time: from2)), endAngle: Angle(degrees: timeToAngle(time: addTimeInString(time1: from2, time2: to2))), clockwise: false)
                        }.fill(Color.yellow)
                        Path { path in
                            path.move(to: CGPoint(x: 180, y: 92.5))
                            path.addArc(center: .init(x: 180, y: 92.5), radius: 75, startAngle: Angle(degrees: timeToAngle(time: from1)), endAngle: Angle(degrees: timeToAngle(time: addTimeInString(time1: from1, time2: to1))), clockwise: false)
                        }.fill(Color.blue)
                        Circle()
